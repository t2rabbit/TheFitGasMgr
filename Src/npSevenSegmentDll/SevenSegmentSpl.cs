...
{
    public class SevenSegmentSpl : UserControl
    {
        static int StNameIndex = 0;
        public SevenSegmentSpl()
        {
            //this.SuspendLayout();
            this.Name = "SevenSegmentspl" + (StNameIndex++);
                    ...
                    if (!success)
                    {
                        try
                        {
                            ...
                            switch (tempString.ToLower()[0])
                            {
                                ...
                                case ' ': customPattern = (int)ValuePattern.None; break;
                                case ':': 
        ...
        { 
            ...
            set 
            {
                System.Diagnostics.Trace.WriteLine(Name+" -" + customPattern + " " + value);
                dotUIOn = value; 
                Invalidate();
            } 
        }
            ...
            if (m_bSpik)
            {
                ...
                if (HadDot)
                {
                    if (showDot)
                    {
                        ...
                        e.Graphics.FillEllipse(dotUIOn ? brushLight : brushDark, gridWidth - 1, gridHeight - elementWidth + 1, elementWidth, elementWidth);
                    }
                    else if (DoubleDot)
                    {
                        System.Diagnostics.Trace.WriteLine(Name + "--" + customPattern + " " + dotUIOn);
        ...
        private void SevenSegment_Load(object sender, EventArgs e)
        {
            ...
            this.Enter += new System.EventHandler(this.SevenSegment_Enter);
            this.Leave += new System.EventHandler(this.SevenSegment_Leave);
        ...
        private void SevenSegment_Enter(object sender, EventArgs e)
        {
            if (t == null)
            {
        ...
        private void SevenSegment_Leave(object sender, EventArgs e)
        {
            if (t != null)
            {
                t.Enabled = false;
        ...
        private void InitializeComponent()
        {
            ...
            // SevenSegmentSpl
            // 
            this.Name = "SevenSegmentSpl";
            this.Load += new System.EventHandler(this.SevenSegmentSpl_Load);
            this.Enter += new System.EventHandler(this.SevenSegmentSpl_Enter);
            this.ResumeLayout(false);

        }

        private void SevenSegmentSpl_Enter(object sender, EventArgs e)
        {

        }

        private void SevenSegmentSpl_Load(object sender, EventArgs e)
        {
