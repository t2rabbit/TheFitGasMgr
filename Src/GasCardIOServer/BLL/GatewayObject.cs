...
namespace IOServer.BLL
{
	class GatewayObject
	{
		...
		//
		#region 单件定义
			protected GatewayObject() { }
			static private GatewayObject m_pInst;
			static public GatewayObject Get()
			{
				if (m_pInst == null)
				{
					m_pInst = new GatewayObject();
				}
				return m_pInst;
			}
		#endregion
		...
		public bool Init(int iGPRSPort)
		{
			...
			//DevModThreadStaticMgr.Get().Init();
			MemNodeValue.Get().Init();
        ...
        private void Run()
        {
            ...
            while (m_bRun)
            {
                try
                {
                    ...
                    //
                    MemNodeValue.Get().UpdateOrInserValToDB();
