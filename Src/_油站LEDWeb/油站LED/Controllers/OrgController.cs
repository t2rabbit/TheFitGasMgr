...
namespace OilStationLED.Controllers
{
    public class OrgController : Controller
    {
        ...
        public ActionResult List(string Method, string Key)
        {
            ...
            try
            {
                using (LedDb db = new LedDb())
                {
                    ...
                    if (Method == "FirstLoad")
                    {
                        list = db.OrgInfo.Where(p => p.IsDel != 1 || p.IsDel == null).ToList();
                    }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Add(GlareSysEfDbAndModels.OrgInfo subject)
        {
            ...
            try
            {
                using (LedDb db = new LedDb())
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        bool b = OrgBll.AddAOrg(ref subject, out strMsg);
                        if (b)
                        {
                            objResult = new ResultHelper()
                            {
            ...
            try
            {
                using (LedDb db = new LedDb())
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            OrgBll.DeleteAOrg(int.Parse(Id[i]), out strMsg);
                        }
                        objResult = new ResultHelper()
                        {
                            Status = true,
                ...
                using (DataSet dsCard = new DataSet())
                {
                    ...
                    if (dsOrg == null || dsOrg.Tables.Count <= 0)
                    {
                        objResult = new ResultHelper()
                        {
                            ...
                            Ret = -1,
                            Obj = null,
                            Msg = "OrgList is null,Please try again later",
                            Desc = "OrgList is null,Please try again later"
                        };
                    }
